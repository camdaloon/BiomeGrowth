...
`kelp_plant` and `cave_vines_plant` are handled internally as part of `kelp` and `cave_vines` and are not separate command identifiers.
...
Changing a rule also updates plants that were already planted before the rule was changed.
