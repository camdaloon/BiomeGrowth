...
Configured growth is adjusted through Bukkit/Paper growth events. Age-based crops receive additional natural random-tick opportunities scaled more aggressively with the configured speed, while tree saplings and other non-ageable plants continue to use extra natural random-tick opportunities. The plugin does not change the global `randomTickSpeed`, so one biome's rule does not affect unrelated plants or biomes.
