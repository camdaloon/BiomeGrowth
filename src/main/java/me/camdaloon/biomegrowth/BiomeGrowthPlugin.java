...
public final class BiomeGrowthPlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    public void onNaturalGrowth(BlockGrowEvent event) {
        ...
        // Track this plant so higher speed values can give it additional
        // natural random-tick opportunities.
        acceleratedPlants.add(BlockKey.of(block));

        int speed = rule.speed();

        // 20 is vanilla speed: leave the event completely untouched.
        // Lower values cancel a percentage of normal growth attempts.
        // Higher values are accelerated by the scheduled random ticks below.
    ...
    public void onSaplingPlaced(BlockPlaceEvent event) {
        ...
        // Track newly planted crops immediately.  Waiting for the first
        // natural growth event made faster crops depend on getting lucky
        // before the accelerator could even start helping them.
    ...
    private void tickAcceleratedPlants() {
        if (!getConfig().getBoolean("enabled", true) || acceleratedPlants.isEmpty()) return;
        ...
        while (iterator.hasNext()) {
            ...
            // Crops need noticeably more opportunities than a single extra
            // random tick per second provides.  Give faster plants a stable
            // number of additional natural random-tick attempts each second.
            // The fractional part is rolled independently so values such as
            // 25 and 75 still scale smoothly.  randomTick() keeps Minecraft's
            // normal crop rules (light, farmland, hydration, etc.) intact.
            ...
            for (int i = 0; i < extraTicks; i++) {
                if (block.getType() != PLANT_BLOCKS.get(plant)) {
                    iterator.remove();
                    break;
                }
                block.randomTick();
