...
public final class BiomeGrowthPlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private static final String ALL_BIOMES = "ALL";
    ...
    private static Map<String, Material> createPlantBlocks() {
        ...
        for (Material material : Material.values()) {
            ...
            if (name.endsWith("_sapling") || name.equals("bamboo") || name.equals("sugar_cane")
                    ...
                    || name.equals("mangrove_propagule") || name.equals("sea_pickle")
                    || name.equals("brown_mushroom") || name.equals("red_mushroom")
                    || name.equals("glow_berries") || name.equals("seagrass")
                    || name.equals("tall_seagrass") || name.equals("firefly_bush")
                    || name.equals("azalea") || name.equals("flowering_azalea")) {
    ...
    private static Map<Material, String> createBlockPlantNames() {
        ...
        map.put(Material.TALL_SEAGRASS, "seagrass");
        return Collections.unmodifiableMap(map);
    }
    ...
    private static final Map<String, String> LEGACY_PLANT_NAMES = Map.ofEntries(
            ...
            Map.entry("tall_seagrass", "seagrass")
    );
    ...
    private boolean isBlockForPlant(String plant, Material material) {
        ...
        if (plant.equals("seagrass")) return material == Material.SEAGRASS || material == Material.TALL_SEAGRASS;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("plantspeedinfo")) {
            if (args.length != 2) {
                player.sendMessage(PREFIX + ChatColor.YELLOW + "Usage: /" + label + " <plantname> <biome>");
                return true;
            }
            ...
            if (biome.equals(ALL_BIOMES.toLowerCase(Locale.ROOT))) {
                player.sendMessage(PREFIX + ChatColor.RED + "Use a specific Overworld biome for /plantspeedinfo.");
                return true;
            }
            ...
            PlantRule rule = rules.get(key(plant, biome));
            if (rule == null) {
                player.sendMessage(PREFIX + ChatColor.YELLOW + plant + " in " + biome + " has no custom speed set and uses vanilla growth.");
            } else {
                player.sendMessage(PREFIX + ChatColor.GREEN + plant + " growth speed in " + biome + " is set to " + rule.speed() + ".");
            }
            return true;
        }
        ...
        if (biome.equals(ALL_BIOMES.toLowerCase(Locale.ROOT))) {
            ...
            for (String overworldBiome : getOverworldBiomes()) {
                rules.put(key(plant, overworldBiome), new PlantRule(plant, overworldBiome, speed));
            }
            saveRules();
            refreshChunkImmediately(player.getChunk());
            queueLoadedChunkRefreshes();
            player.sendMessage(PREFIX + ChatColor.GREEN + "Set " + plant + " growth speed in all Overworld biomes to " + speed + ".");
            return true;
        }
    ...
    private List<String> getOverworldBiomes() {
        return Arrays.stream(org.bukkit.block.Biome.values())
                .map(b -> b.name().toLowerCase(Locale.ROOT))
                .filter(b -> !isSpecialBiome(b))
                .toList();
    }

    private void refreshChunkImmediately(org.bukkit.Chunk chunk) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2) {
            ...
            if (ALL_BIOMES.toLowerCase(Locale.ROOT).startsWith(prefix)) suggestions.add(ALL_BIOMES);
            suggestions.addAll(getOverworldBiomes().stream()
        ...
        if (args.length == 3 && command.getName().equalsIgnoreCase("plantspeedset")) {
